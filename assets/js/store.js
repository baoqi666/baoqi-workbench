...
(function (global) {
 ...
 function defaults() {
  return {
   ...
   reviews: [],
   express: {},
   yuzhuo: { content: {}, remind: {}, steps: [] },   // 玉琢：每日内容覆盖（按日期）+ 每日提醒时间自定义 + 口语训练步骤
   rewards: [],
   redeemLog: [],
   redeemed: 0,
   rewardDismissed: [],
 ...
 function open() {
  ...
  ready = IDB.get(KEY).then(function (raw) {
   if (raw && typeof raw === 'object') {
    ...
    if (!raw.yuzhuo || typeof raw.yuzhuo !== 'object') raw.yuzhuo = { content: {}, remind: {}, steps: [] };
    if (!raw.yuzhuo.content) raw.yuzhuo.content = {};
    if (!raw.yuzhuo.remind) raw.yuzhuo.remind = {};
    if (!raw.yuzhuo.steps) raw.yuzhuo.steps = [];
